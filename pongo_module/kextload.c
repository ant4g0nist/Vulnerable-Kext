...
found:;
	...
	puts("Current Kernelcache UUID");
	for(int i=0; i<16; i++)
		printf("%02x", kernelcache_uuid[i]);
...
kextload_patch() {
	...
	xnu_pf_maskmatch(patchset, "KTRR MMU lockdown", ktrr_mmu_match, ktrr_mmu_mask, ktrr_mmu_count,
			true, ktrr_mmu_patch);
	...
	xnu_pf_maskmatch(patchset, "KTRR AMCC lockdown", ktrr_amcc_match, ktrr_amcc_mask, ktrr_amcc_count,
	...
	xnu_pf_maskmatch(patchset, "OSKext::initWithPrelinkedInfoDict", OSKext_init_match, OSKext_init_mask, OSKext_init_count,
